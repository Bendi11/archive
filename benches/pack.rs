...
fn pack_nocompress(c: &mut Criterion) {
    c.bench_function("Barchive pack (no compression)", move |b| {
        b.iter_with_setup(|| std::io::BufWriter::new(std::fs::File::create("./benches/test-out.bar").unwrap()), |mut file| {
            let mut bar = black_box(Bar::pack("./benches/test-in", Cursor::new(vec![0u8 ; 2048]))).unwrap();
